...
class CodeIndexManager:
    ...
    async def index_project(
        self, project_path: str, patterns: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def _store_entities(self, entities: List[Dict], project_path: str):
        """Store entities in SQLite database using UPSERT to handle re-indexing."""
        from .models import CodeEntity
        from sqlalchemy import delete
        from sqlalchemy.dialects.sqlite import insert as sqlite_insert

        async with self.db.get_session() as session:
            # Clear existing entities for this project first
            await session.execute(
                delete(CodeEntity).where(CodeEntity.project_path == project_path)
            )

            # Use UPSERT (INSERT OR REPLACE) to handle any duplicate IDs
            # This handles edge cases where the delete hasn't fully propagated
            # or where duplicate entity IDs appear in the same batch
            for entity_dict in entities:
                stmt = sqlite_insert(CodeEntity).values(
                    id=entity_dict["id"],
                ...
                # On conflict (duplicate ID), update all fields
                stmt = stmt.on_conflict_do_update(
                    index_elements=["id"],
                    set_={
                        "project_path": stmt.excluded.project_path,
                        "entity_type": stmt.excluded.entity_type,
                        "name": stmt.excluded.name,
                        "qualified_name": stmt.excluded.qualified_name,
                        "file_path": stmt.excluded.file_path,
                        "line_start": stmt.excluded.line_start,
                        "line_end": stmt.excluded.line_end,
                        ...
                        "imports": stmt.excluded.imports,
                        "inherits": stmt.excluded.inherits,
    ...
    async def _store_file_entities(
        ...
        """Store entities for a single file using UPSERT to handle re-indexing."""
        ...
        async with self.db.get_session() as session:
            ...
            # Use UPSERT to handle duplicate IDs safely
            for entity_dict in entities:
                stmt = sqlite_insert(CodeEntity).values(
                    id=entity_dict["id"],
                    project_path=entity_dict["project_path"],
